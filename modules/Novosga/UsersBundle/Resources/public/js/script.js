...
App.Usuarios = {
    ...
    multiDelete: function(btn, selClass, ondelete) {
        if (confirm(App.Usuarios.multiDeleteLabel)) {
            $('.' + selClass + ':checked').each(function(i, e) {
                var row = $(e).parent().parent();
                row.remove();
                ondelete(row);
            });
            App.Usuarios.multiCheck(selClass, btn);
        }
        return false;
    },
    ...
    delLotacoes: function(btn) {
        return App.Usuarios.multiDelete(btn, 'check-lotacao', function(row) {
        });
    },
    
    delServicos: function(btn) {
        return App.Usuarios.multiDelete(btn, 'check-servico', function() {
        });
    },
    ...
    addLotacao: function() {
        ...
        if (grupo.val() > 0 && cargo.val() > 0) {
            ...
            row.find('.check').append('<input type="checkbox" class="check-lotacao" onchange="App.Usuarios.multiCheck(this, \'btn-remove-lotacao\')" />');
            ...
            row.find('.cargo').append('<a href="javascript:void(0)" onclick="App.Usuarios.permissoes(' + cargo.val() + ')" title="' + App.Usuarios.labelVisualizarPermissoes + '">' + cargo.find('option:selected').text() + '</a>');
    ...
    addServico: function() {
        ...
        if (unidade.val() > 0 && servicos.length) {
            for (var i = 0; i < servicos.length; i++) {
                ...
                row.find('.check').append('<input type="checkbox" class="check-servico" onchange="App.Usuarios.multiCheck(this, \'btn-remove-servico\')" />');
    ...
    dialogSenha: function(label) {
        var buttons = {};
        buttons[label] = App.Usuarios.alterarSenha;
