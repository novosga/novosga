...
class Configuracao extends Model implements \JsonSerializable
{
     ...
     * @param type $key
     *
     * @return AppBundle\Entity\Configuracao
     */
    public static function get(EntityManager $em, $key)
    {
        try {
            $query = $em->createQuery("SELECT e FROM AppBundle\Entity\Configuracao e WHERE e.chave = :key");
    ...
    {
        $query = $em->createQuery("DELETE FROM AppBundle\Entity\Configuracao e WHERE e.chave = :key");
        $query->setParameter('key', $key);
