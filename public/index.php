...
$app->post('/install/:action', function ($action) use ($app) {
    ...
    if ($ref->isPublic()) {
        $response = $ref->invokeArgs($controller, [$app->getContext()]);
...
$app->any('/modules/:moduleKey(/:action+)', function ($moduleKey, $action = 'index') use ($app, $loader) {
    ...
    if ($action === 'resources') {
        try {
            $app->moduleResource($moduleKey, implode(DS, array_slice($args, 1)));
        } catch (Exception $e) {
            $app->notFound();
        }
    }
    ...
    // module views
    $app->view()->twigTemplateDirs = [NOVOSGA_TEMPLATES, "$moduleDir/views"];
